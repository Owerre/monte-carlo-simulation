...
class RandomWalk1D:
    ...
    def gaussian_approx(self, const):
        ...
        sigma2 = 4*self.nsteps*self.p*(1-self.p) # exact average for large n
        gau_prob = [0]*len(x_pos)

        for i in  range(len(x_pos)):
            coeff = const/np.sqrt(2*np.pi*sigma2)
            gau_prob[i] = coeff*np.exp( -(x_pos[i] - xbar)**2/(2*sigma2) )
    ...
    def exact_enumeration(self):
        ...
        for x in all_pos:
            ...
            if 2*k % 2 == 0:
                ...
                dem = math.factorial(self.nsteps - int(k))*math.factorial(int(k))
                bino_coeff = math.factorial(self.nsteps)/dem
                for _ in np.arange(bino_coeff):
                    actual_pos.append(x)
        
        # compute averages            
        x_avg = sum([x for x in actual_pos])/2**self.nsteps # 0 for p = 0.5
        x2_avg = sum([x*x for x in actual_pos])/2**self.nsteps # nsteps for p = 0.5
